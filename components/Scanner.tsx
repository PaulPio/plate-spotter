...
const Scanner: React.FC<ScannerProps> = ({ onScanComplete, onCancel }) => {
  ...
  const handleCameraCapture = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error(err);
      setProcessing({ status: 'error', message: 'Error processing image. Plase try again.' });
    }
  };
  ...
  const handleFinalSave = () => {
    const result: ScanResult = {
      ...
      timestamp: new Date().toLocaleString('en-US', { timeZone: 'America/New_York', month: '2-digit', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }) + ' ET',
      method: activeTab === 'camera' ? 'camera' : 'manual',
      entryType: 'repair',
      serviceDetails: serviceDetails, // Required for Repair
      companyName: '', // Ensure field exists for Google Sheet
      ...tempScanData
    };
    ...
    <div className="fixed inset-0 bg-slate-900/95 z-50 flex flex-col animate-fade-in">
      ...
      <div className="flex justify-between items-center p-4 border-b border-slate-700">
        <h2 className="text-lg font-semibold text-white">
          {scanStep === 'capture' ? 'New Service Entry' : 'Review & Save'}
        </h2>
          ...
          <div className="flex flex-col h-full space-y-5">
            ...
            <div className="flex-1">
              ...
              <textarea
                ...
                placeholder="e.g. Oil change, Brake pad replacement, 50k checkup..."
            ...
            <button
              ...
              disabled={!plateInput.trim() || !serviceDetails.trim() || processing.status === 'success'}
